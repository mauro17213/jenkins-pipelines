 ...
 * @author pavacca
 */
@FacesConverter("ipsConverterAusCaso")
public class IpsConverterAusCaso implements Converter{
    @Override
            ...
            try {
                ...
                for(Maestro _obj : service.getListaTipoServicioIPS()){
                    if(Integer.parseInt(value) == _obj.getId()){
                        obj = _obj;
                        service.getServicioParaCrear().setObjetoIps(new Maestro());
                        service.getServicioParaCrear().getObjetoIps().setId(_obj.getId());
                        break;
                    }
                }
