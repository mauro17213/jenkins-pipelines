...
public class AuSolicitudServicioImpl extends AccionesBO implements AuSolicitudServicioIface {
    ...
    private final static String MENSAJE_RECHAZO_ITEM = "El ítem fue rechazado ya que se encuentra capitado por la ips primaria del afiliado";
...
//                                anexo4Estado.setId(idAnexo4Estado);
    ...
    public void AnularAuditoria(AuSolicitudBean bean) {
        try {
            if (validarEstadoItem(bean)) {
                int estadoAnulado = AuAnexo3Item.ESTADO_NEGADO_AUDITORIA;
                Maestro motivoAnulado = bean.obtenerMaestroAnulado(bean.getObjeto().getIdMotivo());
...
//            }
    ...
    public AuAnexo3Afiliado casteoAuAnexo3Afiliado(AsegAfiliado afiliado) {
        ...
        afiliadoAnexo.setMaeRegimenId(Integer.parseInt(afiliado.getRegimen()));
...
//                listaTutela = servicioTutela.consumoServicioTutela(tipoDocumento, bean.getObjeto().getAsegAfiliadoId().getNumeroDocumento());
        listaTutela = getTutelaRemoto().consultarConExoneracionAfiliadoPorId(bean.getObjeto().getAsegAfiliadoId().getId(), Integer.parseInt(PropTutelasUsuario.getInstance().get(PropTutelasUsuario.REST_TIPO_ESTADO_FALLO)));
