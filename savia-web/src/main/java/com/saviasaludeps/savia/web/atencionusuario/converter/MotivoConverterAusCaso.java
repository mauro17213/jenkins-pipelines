 ...
 * @author pavacca
 */
@FacesConverter("motivoConverterAusCaso")
public class MotivoConverterAusCaso implements Converter{
    @Override
    public Maestro getAsObject(FacesContext context, UIComponent component, String value) {
        ...
        if(value != null && value.trim().length() > 0) {
            try {
                ...
                for(Maestro _obj : service.getListaTipoServicioMotivo()){
                    if(Integer.parseInt(value) == _obj.getId()){
                        obj = _obj;
                        service.getServicioParaCrear().setMaeServicioMotivoId(_obj.getId());
                        break;
                    }
                }
