 ...
 * @author pavacca
 */
@FacesConverter("tipoServicioConverterAusCaso")
public class TipoServicioConverterAusCaso implements Converter{
    @Override
    public Maestro getAsObject(FacesContext context, UIComponent component, String value) {
        ...
        if(value != null && value.trim().length() > 0) {
            try {
                ...
                for(Maestro _obj : service.getListaTipoServicio()){
                    if(Integer.parseInt(value) == _obj.getId()){
                        obj = _obj;
                        service.getServicioParaCrear().setMaServicioId(_obj.getId());
                        service.getServicioParaCrear().setMaServicioCodigo(_obj.getValor());
                        service.getServicioParaCrear().setMaServicioValor(_obj.getNombre());
                        break;
                    }
                }
