 ...
 * @author pavacca
 */
@FacesConverter("ambitoServicioConverterAusCaso")
public class AmbitoServicioConverterAusCaso implements Converter{
    @Override
    public Maestro getAsObject(FacesContext context, UIComponent component, String value) {
        ...
        if(value != null && value.trim().length() > 0) {
            try {
                ...
                for(Maestro _obj : service.getListaTipoServicioAmbito()){
                    if(Integer.parseInt(value) == _obj.getId()){
                        obj = _obj;
                        service.getServicioParaCrear().setMaeServicioAmbitoId(_obj.getId());
                        break;
                    }
                }
