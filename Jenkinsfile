pipeline {
    ...
    environment {
        ...
        MAVEN_FLAGS  = "-Dmaven.test.failure.ignore=true -Dmaven.repo.local=${WORKSPACE}/.m2/repository"
    }
    ...
    stages {
        stage('Build en Linux') {
            ...
            steps {
                ...
                '''
                // Guardar el EAR para usarlo en Windows
                stash includes: 'savia-ear/target/*.ear', name: 'earFile'
            }
        }
        ...
        stage('Deploy en Windows') {
            agent { label 'Windows' }
            steps {
                // Recuperar el EAR que se generó en Linux
                unstash 'earFile'
