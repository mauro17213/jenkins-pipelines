 ...
 * @author pavacca
 */
@FacesConverter("diagnosticoConverterAusCaso")
public class DiagnosticoConverterAusCaso implements Converter{
   @Override
    public Maestro getAsObject(FacesContext context, UIComponent component, String value) {
        ...
        if(value != null && value.trim().length() > 0) {
            try {
                ...
                for(Maestro _obj : service.getListaTipoDiagnostico()){
                    if(Integer.parseInt(value) == _obj.getId()){
                        obj = _obj;
                        service.getServicioParaCrear().setMaDiagnostico(new MaDiagnostico());
                        service.getServicioParaCrear().getMaDiagnostico().setId(_obj.getId()); 
                        break;
                    }
                }
