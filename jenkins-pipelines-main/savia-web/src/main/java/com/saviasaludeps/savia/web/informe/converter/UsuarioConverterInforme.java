 ...
 * @author ripalacios
 */
@FacesConverter("usuarioConverterInforme")
public class UsuarioConverterInforme implements Converter {
    ...
    public Usuario getAsObject(FacesContext context, UIComponent component, String value) {
        ...
        if(value != null && value.trim().length() > 0) {
            try {
                ...
                InformeGrupoBean service = (InformeGrupoBean) viewMap.get("informeGrupoBean");
                for(Usuario _obj : service.getUsuarioRecursiva().values()){
