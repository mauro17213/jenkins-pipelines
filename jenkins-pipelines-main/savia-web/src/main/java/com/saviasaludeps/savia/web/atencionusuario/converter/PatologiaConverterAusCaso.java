 ...
 * @author pavacca
 */
@FacesConverter("patologiaConverterAusCaso")
public class PatologiaConverterAusCaso implements Converter{
    @Override
    public Object getAsObject(FacesContext context, UIComponent component, String value) {
        ...
        if(value != null && value.trim().length() > 0) {
            try {
                ...
                for(Maestro _obj : service.getListaTipoPatologia()){
                    if(Integer.parseInt(value) == _obj.getId()){
                        obj = _obj;
                        service.getServicioParaCrear().setMaePatologiaId(_obj.getId());
                        break;
                    }
                }
